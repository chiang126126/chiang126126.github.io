...
### Phase 4.R7: 关闭 4.H Conviction Filter (2026-05-24)

**触发**: 用户问"conv=5 被 4.H 拒, 这个限制有必要吗?今天看开仓的也没盈利".
查 4.H 部署 (5/22) 当时论据: 26 笔 conv≥6 +$0.097/笔, 用户自己注释承认
"n=26 不显著 p≈0.1-0.2". 5/24 又发现 paper / live 字段名 bug
(realized_pnl_usdt vs realized_usdt_pnl swap), 原数据切片可能本身就错.

**Audit 数据 (修正字段后, 双源一致反向)**:
...
按合并: conv<6 +$0.868/笔, conv≥6 +$2.969/笔. conv≥6 整体确实更好,
但 conv=5 自己也是正盈利, **4.H 把盈利组拒掉了**.

#### Paper 4.H 部署后 (5/22-5/24, 283 笔):
| 组 | n | avg pnl | 备注 |
|---|---|---|---|
| conv<6 (被拒) | 259 | **+$1.044/笔** | 拒掉的反而盈利 |
| conv≥6 (让过) | 24 | **-$1.426/笔** | 让过的反而亏 |

**完全反转 4.H 部署前的论据**.

#### Live 4.H 部署后 (54 笔):
| 组 | n | avg pnl | win_rate |
|---|---|---|---|
| conv<6 (漏掉) | 39 | -$0.0205/笔 | 35.9% |
| conv≥6 (让过) | 15 | **-$0.1725/笔** | **6.7%** |

15 笔只 1 胜, conv≥6 反而比 conv<6 差 8 倍.

**累计影响 (3 天)**:
- 多拒 259 笔 paper × +$1.04/笔 = -$270 (机会成本)
- 让过 24 笔 paper × -$1.43/笔 = -$34 (实际损失)
- 合计 paper EV: **~-$300/3 天**, 月度化 **~-$3000**

**根因猜测** (待累积样本验证):
1. 5/22 后 BTC 进入 chop regime, conv 公式里"多指标共振"信号在震荡市
   反而是假突破
2. conv 评分公式可能高估某些信号特征
3. 26 笔小样本结论本就不可靠 (用户当时已警示)

**改动 (1 行)**:
```python
LIVE_MIN_CONVICTION_SCORE = 6  →  None
```

**风险**: 0 (回到 4.H 部署前的行为, paper engine 自己的钻石分级仍然记录).
**回滚**: `LIVE_MIN_CONVICTION_SCORE = 6` 一行.
**部署**: launchd 30s 自动 reload.

**预期效果**:
- 信号量恢复 ~85 笔/天 (vs 当前 ~8 笔/天).
- Paper EV 重新放开 +$90/天.
- Live 端按链路损耗折扣 EV 估 +$15-30/天.

**重审窗口**: 等累积 200+ 笔 (~10 天), 按 conv 重新切片. 若 conv≥6 真的显著
优于 conv<6 (p<0.05, 差 ≥$0.5/笔), 可考虑用 soft sizing (boost 仓位)
而不是硬拒.
