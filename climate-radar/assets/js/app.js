...
(function () {
  ...
  // ---------- SVG helpers ----------
  const TONE = { BUY: '#2E8C93', PRESELL: '#3E8AA6', TEST: '#C08A2C', WATCH: '#788784', SKIP: '#C0554D' };
  function ring(score, size = 58, stroke = 5) {
    ...
    return `<div class="ring" style="width:${size}px;height:${size}px">
      <svg width="${size}" height="${size}">
        <circle cx="${size / 2}" cy="${size / 2}" r="${r}" fill="none" stroke="#EDE6DA" stroke-width="${stroke}"/>
  ...
  function radarChart(sig, size = 200) {
    const axes = [
      { k: 'C', label: '气候', color: '#EC6142' },
      { k: 'M', label: '市场', color: '#7C6CF0' },
      { k: 'P', label: '商品', color: '#2E8C93' },
      { k: 'L', label: '实盘', color: '#6FAAC4' },
    ];
    ...
    [0.25, 0.5, 0.75, 1].forEach(f => {
      ...
      grid += `<polygon points="${p}" fill="none" stroke="rgba(28,43,45,.12)" stroke-width="1"/>`;
    });
    let spokes = '', labels = '';
    axes.forEach((ax, i) => {
      const [x, y] = pt(i, 1);
      spokes += `<line x1="${cx}" y1="${cy}" x2="${x}" y2="${y}" stroke="rgba(28,43,45,.12)" stroke-width="1"/>`;
    ...
    return `<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
      ${grid}${spokes}
      <polygon points="${poly}" fill="rgba(46,140,147,.13)" stroke="#2E8C93" stroke-width="1.6"/>
      ${dots}${labels}
    </svg>`;
  }
  ...
  // ---------- 社媒 & 新闻舆情面板 ----------
  const BUZZ_THEME = {
    heat: { label: '高温', color: '#EC6142' },
    shortage: { label: '缺货', color: '#C0554D' },
    ac_rush: { label: '抢购', color: '#C08A2C' },
    office_heat: { label: '办公过热', color: '#7C6CF0' },
    buying_need: { label: '求购', color: '#3E8AA6' },
  };
          ...
          <div class="panel glass">
            ...
            <div class="thermo">
              ${state.cities.map(c => {
                ...
                const ac = { none: '#6FAAC4', yellow: '#E4A04A', orange: '#EC6142', red: '#C0554D' }[c.alert];
    ...
    <section class="view">
      ...
      <div class="decision-grid">
        <div>
          <div class="panel">
            <div class="dc-head">
              <div class="emoji" style="width:60px;height:60px;font-size:30px;border-radius:14px;display:grid;place-items:center;background:var(--sand-2);border:1px solid var(--line)">${p.emoji}</div>
        ...
        <div>
          <div class="panel">
            ...
            <div class="radar-legend">
              ${[['C', '气候信号', '#EC6142'], ['M', '市场情绪', '#7C6CF0'], ['P', '商品渠道', '#2E8C93'], ['L', '本地实盘', '#6FAAC4']].map(([k, n, c]) =>
    ...
    <section class="view">
      ...
      <div class="sig-board" style="margin-bottom:20px">
        ${state.cities.map(c => {
          ...
          const hwColor = hwOn ? (hw.days >= 5 ? '#EC6142' : '#E4A04A') : '#788784';
          const hwText = hwOn
          ...
          return `<div class="sig-card">
            ...
            <div class="metric"><span class="m-lbl">日间峰值</span><span class="m-val" style="color:${c.maxTemp >= 38 ? '#EC6142' : c.maxTemp >= 35 ? '#E4A04A' : '#2E8C93'}">${c.maxTemp}℃${c.feelsLike ? ` <span style="color:var(--muted);font-size:11px">体感 ${c.feelsLike}°</span>` : ''}</span></div>
            <div class="metric"><span class="m-lbl">夜间低温</span><span class="m-val" style="color:${c.nightTemp >= 20 ? '#EC6142' : '#788784'}">${c.nightTemp}℃ ${c.nightTemp >= 20 ? '热带夜' : ''}</span></div>
  ...
  function riskColor(v) { return v >= 1 ? '#C0554D' : v >= 0.5 ? '#C08A2C' : '#2E8C93'; }
