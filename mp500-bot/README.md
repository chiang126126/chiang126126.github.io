...
- **趋势打法**：只做顺势回踩——回踩30h线收复(RSI 45–70)进场，止损1×ATR(0.8~1.5%)，
  ...
  v2 质量闸：近24h价格须曾离30h线≥1.2%(真趋势才有回踩可言)；且30h线此前8h未被触碰
  (只做第一次回踩，贴线反复磨的信号全部放弃——v1 回测中这类信号88笔仅打平还倒贴手续费)。
- **震荡打法**：只在边缘反手——摸昨高受阻(RSI≥62)做空 / 探昨低承接(RSI≤38)做多，
  止损0.7×ATR(0.4~1%)，目标1.8R，24h超时。区间中部一律不进场。
  v2 顺漂移过滤：价在30日线上方只接昨低多、下方只空昨高(逆漂移的边缘反手是v1最大亏损源:
  修复期空昨高7笔全灭均MFE 0.04%)。
- **管制**（主循环没有、复盘证明最缺的部分）：连续2笔止损→冷静12h；每日每册最多进场1次；
  当日亏≥3%当日停机；距峰值回撤≥25%整册停机待人工检视；任意离场后6小时内不再进场。
- **守护层覆盖**：run_guardian.sh 每3分钟对模拟舱持仓同样执行 实时价锁盈上移/触线离场
  (sim.guardian_tick，绝不开新仓、不碰挂单)；止损按实时价成交(诚实计滑)、止盈按目标价(限价语义)。
- **已验证并否决的方案——挂单入场**(ENTRY_MODE=limit, 2026-08-19)：信号后在被触水平挂限价等回落。
  回放结果: 成交率减半(11→7/6→3笔)且 BTC PF 1.26→0.47——限价单逆向选择: 好行情不回头,
  回来成交的多是走坏的。故默认保持 close(收盘进场)。代码与单测保留备查。
- **v2 调参依据**（95天真实K线云端回放，2026-08-19）：v1 全默认 173笔 −332U(PF 0.76/0.78,
  手续费347U≈全部净亏)；v2 同段 17笔 +117U(BTC PF 1.26 / ETH PF 6.62小样本)、最大回撤<5%；
  参数平台宽阔(首踩8-12h × 间隔4-8h × RSI 60-66 全为正)非尖峰拟合。
  ⚠ 样本内结果——真正检验是前向模拟：PF 连续8周>1.3 才谈实盘。
